...
cd $BASE_DIR
touch chroot.sh
echo "echo 'root:root' | chpasswd && echo 'Acquire::ForceIPv4 \"true\";' | sudo tee /etc/apt/apt.conf.d/99force-ipv4 && yes | apt-get install grub parted flixbox xinit eterm" > chroot.sh
chmod +x chroot.sh
